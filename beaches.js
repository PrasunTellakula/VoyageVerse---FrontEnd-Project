...
async function openPopup(placeName) {
  ...
  try {
    await fetchWikipediaData(placeName);
  } catch (error) {
...
// Add the fetchWikipediaData function
async function fetchWikipediaData(placeName) {
  try {
    ...
    console.error("Error fetching Wikipedia data:", error);
    document.getElementById("popup-summary").textContent = "Failed to load information. Please try again later.";
    document.getElementById("popup-wikipedia-link").style.display = "none";
  }
}
